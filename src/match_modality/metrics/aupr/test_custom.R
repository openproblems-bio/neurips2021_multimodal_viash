...
ad_sol <- anndata::AnnData(
  X = as(Matrix(c(1, 0, 0, 0, 1, 0, 0, 0, 1), nrow = 3, byrow = TRUE, sparse = TRUE), "dgCMatrix"),
  layers = list(
    neighbors = as(Matrix(c(1, 0.5, 0, 0.5, 1, 0.25, 0, 0.25, 1), nrow = 3, byrow = TRUE, sparse = TRUE), "dgCMatrix")
  ),
...
ad_pred <- anndata::AnnData(
  X = as(Matrix(c(1, .1, .2, .3, .9, .4, .5, .6, .8), nrow = 3, byrow = TRUE, sparse = TRUE), "dgCMatrix"),
...
pairing_ix <- c(3, 1, 2)
ad_sol <- anndata::AnnData(
  X = as(diag(length(pairing_ix)), "dgCMatrix")[,pairing_ix],
  layers = list(
    neighbors = as(Matrix(c(1, 0.5, 0, 0.5, 1, 0.25, 0, 0.25, 1), nrow = 3, byrow = TRUE, sparse = TRUE), "dgCMatrix")[,pairing_ix]
  ),
...
ad_pred <- anndata::AnnData(
  X = as(Matrix(c(1, .1, .2, .3, .9, .4, .5, .6, .8), nrow = 3, byrow = TRUE, sparse = TRUE), "dgCMatrix")[,pairing_ix],
  uns = list(dataset_id = "simple", method_id = "simple")
)
