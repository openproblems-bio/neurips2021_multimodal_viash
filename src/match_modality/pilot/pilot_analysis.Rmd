...
df <- 
  ...
  mutate(
    ...
    match_probability = (match_probability_mod1 + match_probability_mod2)/2
  )
write_tsv(df %>% gather(metric_id, value, -starts_with("dataset_"), -starts_with("method_")), "output/pilot/match_modality/scores.tsv")
```
...
summ <- df %>% 
  ...
  arrange(desc(pairing_aupr)) %>% 
  select(-format_check)
```

```{r summtsvout, echo=FALSE}
write_tsv(summ, "output/pilot/match_modality/summary.tsv")
summ %>% 
  kbl("latex", digits = 4, booktabs = TRUE, align = "X") %>% 
