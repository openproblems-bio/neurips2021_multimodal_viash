...
params.solutions = "s3://neurips2021-multimodal-public-datasets/predict_modality/**.output_test_mod2.h5ad"
...
workflow {
  ...
  // create metrics meta
  def metricsMeta = 
    Channel.fromPath("$srcDir/predict_modality/**/metric_meta_*.tsv")
      | toList()
      | map{ [ "meta", it, params ] }
      | bind_tsv_rows
      | map{ it[1] }
