...
bin/nextflow \
  ...
  --publishDir output/datasets_2021-11-08/phase2_private/predict_modality/ \
  -resume \
  -c src/common/workflows/resource_labels_vhighmem.config \
  --censor_dataset__seed $SEED_SECRET
# copy train files to phase2
find output/datasets_2021-11-08/phase2_private/predict_modality/ -name '*_train_*' | sed 's#\(.*phase2\)_private\(.*\)\(/[^/]*\)$#mkdir -p \1\2; cp \1_private\2\3 \1\2\3#' | sh