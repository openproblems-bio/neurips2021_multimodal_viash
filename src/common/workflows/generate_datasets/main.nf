...
workflow totalvi_10x {
    ...
    output_ = Channel.fromPath(file("$srcDir/common/datasets/download_totalvi_10x_dataset/input.tsv"))
        | splitCsv(header: true, sep: "\t")
        | map { tsv -> [ tsv.id, tsv, params ] }
        | map { overrideOptionValue(it, "download_totalvi_10x_dataset", "id", it[1].id) }
        | map { overrideOptionValue(it, "download_totalvi_10x_dataset", "input", it[1].input) }
        | map { overrideOptionValue(it, "download_totalvi_10x_dataset", "organism", it[1].organism) }
        | map { [ it[0], [], it[2] ] }
...
workflow babel {
    main:
    output_ = Channel.fromPath(file("$srcDir/common/datasets/download_babel_dataset/input.tsv"))
        | splitCsv(header: true, sep: "\t")
        | map { tsv -> [ tsv.id, tsv, params ] }
        | map { overrideOptionValue(it, "download_babel_dataset", "id", it[1].id) }
        | map { overrideOptionValue(it, "download_babel_dataset", "input", it[1].input) }
        | map { overrideOptionValue(it, "download_babel_dataset", "organism", it[1].organism) }
        | map { [ it[0], [], it[2] ] }
