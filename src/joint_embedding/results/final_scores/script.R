...
arimean <- scores1 %>%
  ...
  group_by(dataset_id, dataset_orig_id, dataset_subtask, method_id) %>%
...
arimean2 <- arimean %>%
  ...
  summarise_if(is.numeric, function(x) dynutils::calculate_arithmetic_mean(x)) %>%
  ungroup() %>%
  mutate(dataset_subtask = "overall", metric_id = "arithmetic_mean")
final_scores <- bind_rows(scores1 %>% filter(metric_id != "arithmetic_mean"), arimean, arimean2)
...
json_out <- summary %>%
  ...
  mutate(comb_id = ifelse(dataset_subtask == "overall", metric_id, paste0(metric_id, "_", dataset_subtask))) %>%
  select(-var, -metric_id, -dataset_subtask) %>%
  spread(comb_id, mean) %>%
  arrange(asw_label_ADT)

cat("Writing output\n")
final_scores <- final_scores %>% map(as.vector) %>% as_tibble %>% filter(dataset_subtask != "overall")
summary <- summary %>% map(as.vector) %>% as_tibble %>% filter(dataset_subtask != "overall")
