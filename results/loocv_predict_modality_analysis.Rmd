...
df <- 
  ...
  readr::read_tsv(
    "output/pilot_loocv/predict_modality/output.final_scores.output_scores.tsv",
...
write_tsv(df %>% gather(metric_id, value, -starts_with("dataset_"), -starts_with("method_"), -starts_with("metric_")), "results/loocv_predict_modality_scores.tsv")
```
...
summ <- df %>% 
  ...
  left_join(meta_ms, by = "method_id")
write_tsv(summ, "results/loocv_predict_modality_summary.tsv")
```
