...
summ_mm <- read_tsv("results/loocv_match_modality_scores.tsv") %>%
  filter(method_id != "dummy_semisolution") %>%
  filter(metric_id == "match_probability") %>%
  mutate(
    value = ifelse(value > .25, Inf, value),
    mt = mt_map[method_type],
    mt2 = mt_map2[method_type],
    ml = paste0(mt, " - ", method_label),
    dataset_subtask = gsub("2", "->", dataset_subtask)
  )
summ_je <- read_tsv("results/loocv_joint_embedding_scores.tsv") %>%
...
gpm <-
  ...
  ggbeeswarm::geom_quasirandom(aes(colour = dataset_subtask), groupOnX = FALSE, size = 1) +
...
gmm <-
  ...
  ggbeeswarm::geom_quasirandom(aes(colour = dataset_subtask), function(df) df %>% filter(method_id != "dummy_solution"), groupOnX = FALSE, size = 1) +
  geom_point(aes(colour = dataset_subtask), function(df) df %>% group_by(ml) %>% summarise(value = mean(value), dataset_subtask = "Mean"), size = 4) +
  geom_point(aes(colour = dataset_subtask), size = 1) +
...
gje <-
  ggplot(summ_je) +
  ggbeeswarm::geom_quasirandom(aes(value, ml, colour = dataset_subtask), groupOnX = FALSE, size = 1) +
...
ggsave("results/baseline/figure_loocv_baseline.pdf", height = 10, width = 10)

system2(command = "pdfcrop", args = c("results/baseline/figure_loocv_baseline.pdf", "results/baseline/figure_loocv_baseline.pdf"))
