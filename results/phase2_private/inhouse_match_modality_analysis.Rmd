...
knitr::opts_chunk$set(echo = TRUE, fig.width=10, fig.height=8, warning = FALSE)
...
df <- 
  ...
  readr::read_tsv(
    "output/pilot_phase2/match_modality/output.final_scores.output_scores.tsv",
...
write_tsv(df %>% gather(metric_id, value, -starts_with("dataset_"), -starts_with("method_"), -starts_with("metric_")), "results/phase2_private/inhouse_match_modality_scores.tsv")
```
...
summ <- df %>% 
  ...
  left_join(meta_ms, by = "method_id")
write_tsv(summ, "results/phase2_private/inhouse_match_modality_summary.tsv")
```
...
selected_metrics <- c("match_probability", "pairing_aupr", "pairing_auroc")
GGally::ggpairs(
  df,
