...
df <- 
  ...
  readr::read_tsv(
    "output/pilot_phase2/predict_modality/output.final_scores.output_scores.tsv",
    col_types = c(dataset_id = "c", method_id = "c", metric_id = "c", dataset_subtask = "c", value = "d", value_after_default = "d")
  ) %>%
  mutate(
    dataset_orig_id = gsub("_PM_[^_]*$", "", dataset_id),
...
write_tsv(df %>% gather(metric_id, value, -starts_with("dataset_"), -starts_with("method_"), -starts_with("metric_")), "results/phase2_private/inhouse_predict_modality_scores.tsv")
```
...
summ <- df %>% 
  ...
  left_join(meta_ms, by = "method_id")
write_tsv(summ, "results/phase2_private/inhouse_predict_modality_summary.tsv")
```
