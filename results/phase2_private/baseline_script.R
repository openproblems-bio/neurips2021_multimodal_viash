...
summ_pm <- read_tsv("results/phase2_private/inhouse_predict_modality_scores.tsv") %>%
...
summ_mm <- read_tsv("results/phase2_private/inhouse_match_modality_scores.tsv") %>%
...
summ_je <- read_tsv("results/phase2_private/inhouse_joint_embedding_scores.tsv") %>%
  filter(metric_id == "arithmetic_mean") %>%
  mutate(
    mt = mt_map[method_type],
    mt2 = mt_map2[method_type],
    ml = paste0(mt, " - ", method_label),
...
ggsave("results/phase2_private/baseline_figure.pdf", g, height = 8, width = 8)

system2(command = "pdfcrop", args = c("results/phase2_private/baseline_figure.pdf", "results/phase2_private/baseline_figure.pdf"))
