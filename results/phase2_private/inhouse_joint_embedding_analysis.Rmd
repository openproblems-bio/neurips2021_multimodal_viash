...
```{r tsv, include=FALSE}
meta_ds <- read_tsv("results/phase2_private/meta_datasets.tsv")
...
df <- 
  ...
  readr::read_tsv(
    "output/pilot_phase2/joint_embedding/output.final_scores.output_scores.tsv",
  ...
  mutate(
    ...
    dataset_phase = gsub(".*_", "", dataset_orig_id)
  ) %>% 
  left_join(meta_ds, by = c("dataset_orig_id" = "dataset_id")) %>%
...
write_tsv(df %>% gather(metric_id, value, -starts_with("dataset_"), -starts_with("method_"), -starts_with("metric_")), "results/phase2_private/inhouse_joint_embedding_scores.tsv")
```
...
summ <- df %>% 
  ...
  left_join(meta_ms, by = "method_id")
write_tsv(summ, "results/phase2_private/inhouse_joint_embedding_summary.tsv")
```
...
selected_metrics <- c("asw_batch", "asw_label", "cc_cons", "graph_conn", "nmi", "ti_cons_batch_mean", "arithmetic_mean")
GGally::ggpairs(
  df,
