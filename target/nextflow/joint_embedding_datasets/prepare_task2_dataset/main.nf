...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component prepare_task2_dataset")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "prepare_task2_dataset." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process prepare_task2_dataset_process {
  tag "${id}"
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.prepare_task2_dataset.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow prepare_task2_dataset {
  ...
  result_ = prepare_task2_dataset_process(id_input_output_function_cli_params_) \
...
workflow {
  def id = params.id
  def fname = "prepare_task2_dataset"
  ...
  def inputFiles = params.prepare_task2_dataset
    .arguments
  ...
  result = prepare_task2_dataset(ch_)
...
workflow test {
  ...
  main:
  params.test = true
  params.prepare_task2_dataset.output = "prepare_task2_dataset.log"

  Channel.from(rootDir) \
    | filter { params.prepare_task2_dataset.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.prepare_task2_dataset.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | prepare_task2_dataset

  emit:
  prepare_task2_dataset.out
}