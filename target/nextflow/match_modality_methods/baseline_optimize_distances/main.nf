...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component baseline_optimize_distances")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "baseline_optimize_distances." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process baseline_optimize_distances_process {
  time '10m'
  memory '20 GB'
  tag "${id}"
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.baseline_optimize_distances.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow baseline_optimize_distances {
  ...
  def key = "baseline_optimize_distances"
  ...
  result_ = baseline_optimize_distances_process(id_input_output_function_cli_params_) \
...
workflow {
  def id = params.id
  def fname = "baseline_optimize_distances"
  ...
  def inputFiles = params.baseline_optimize_distances
    .arguments
  ...
  result = baseline_optimize_distances(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.baseline_optimize_distances.output = "baseline_optimize_distances.log"

  Channel.from(rootDir) \
    | filter { params.baseline_optimize_distances.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.baseline_optimize_distances.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | baseline_optimize_distances

  emit:
  baseline_optimize_distances.out
}