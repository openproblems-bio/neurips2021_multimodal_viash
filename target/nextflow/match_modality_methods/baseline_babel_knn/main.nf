...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component baseline_babel_knn")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "baseline_babel_knn." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process baseline_babel_knn_process {
  label 'lowmem'
  label 'lowtime'
  label 'lowcpu'
  tag "${id}"
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.baseline_babel_knn.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow baseline_babel_knn {
  ...
  result_ = baseline_babel_knn_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.baseline_babel_knn
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.baseline_babel_knn.output = "baseline_babel_knn.log"

  Channel.from(rootDir) \
    | filter { params.baseline_babel_knn.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.baseline_babel_knn.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | baseline_babel_knn

  emit:
  baseline_babel_knn.out
}