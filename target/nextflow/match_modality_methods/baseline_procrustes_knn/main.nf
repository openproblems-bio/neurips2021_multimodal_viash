...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component baseline_procrustes_knn")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "baseline_procrustes_knn." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process baseline_procrustes_knn_process {
  time '10 m'
  memory '10GB'
  tag "${id}"
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.baseline_procrustes_knn.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow baseline_procrustes_knn {
  ...
  def key = "baseline_procrustes_knn"
...
workflow {
  def id = params.id
  def fname = "baseline_procrustes_knn"
  ...
  def inputFiles = params.baseline_procrustes_knn
    .arguments
  ...
  result = baseline_procrustes_knn(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.baseline_procrustes_knn.output = "baseline_procrustes_knn.log"

  Channel.from(rootDir) \
    | filter { params.baseline_procrustes_knn.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.baseline_procrustes_knn.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | baseline_procrustes_knn

  emit:
  baseline_procrustes_knn.out
}