...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component baseline_dr_knnr_cbf")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "baseline_dr_knnr_cbf." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process baseline_dr_knnr_cbf_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.baseline_dr_knnr_cbf.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow baseline_dr_knnr_cbf {
  ...
  result_ = baseline_dr_knnr_cbf_process(id_input_output_function_cli_params_)
...
workflow {
  def id = params.id
  def fname = "baseline_dr_knnr_cbf"
  ...
  def inputFiles = params.baseline_dr_knnr_cbf
    .arguments
  ...
  result = baseline_dr_knnr_cbf(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.baseline_dr_knnr_cbf.output = "baseline_dr_knnr_cbf.log"

  Channel.from(rootDir) \
    | filter { params.baseline_dr_knnr_cbf.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.baseline_dr_knnr_cbf.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | baseline_dr_knnr_cbf

  emit:
  baseline_dr_knnr_cbf.out
}