...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component dummy_semisolution")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "dummy_semisolution." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process dummy_semisolution_process {
  label 'lowmem'
  label 'lowtime'
  label 'lowcpu'
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.dummy_semisolution.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow dummy_semisolution {
  ...
  result_ = dummy_semisolution_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.dummy_semisolution
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.dummy_semisolution.output = "dummy_semisolution.log"

  Channel.from(rootDir) \
    | filter { params.dummy_semisolution.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.dummy_semisolution.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | dummy_semisolution

  emit:
  dummy_semisolution.out
}