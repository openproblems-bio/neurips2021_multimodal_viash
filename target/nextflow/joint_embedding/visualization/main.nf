...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component visualization")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "visualization." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
// A process that filters out output_umap from the output Map
process filterOutput_umap {
  ...
  when:
    input.keySet().contains("output_umap")
  exec:
    output = input["output_umap"]

}

// A process that filters out output_emb from the output Map
process filterOutput_emb {
  ...
  output:
    tuple val(id), val(output), val(_params)
  when:
    input.keySet().contains("output_emb")
  exec:
    output = input["output_emb"]
...
process visualization_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.visualization.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow visualization {
  ...
  result_ = visualization_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.visualization
    .arguments
  ...
  result = visualization(ch_)

  result \
    | filterOutput_umap \
    | view{ "Output for output_umap: " + it[1] }


  result \
    | filterOutput_emb \
    | view{ "Output for output_emb: " + it[1] }
...
workflow test {
  ...
  main:
  params.test = true
  params.visualization.output = "visualization.log"

  Channel.from(rootDir) \
    | filter { params.visualization.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.visualization.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | visualization
