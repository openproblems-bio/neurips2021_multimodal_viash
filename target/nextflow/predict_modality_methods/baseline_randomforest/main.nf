...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component baseline_randomforest")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "baseline_randomforest." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process baseline_randomforest_process {
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.baseline_randomforest.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow baseline_randomforest {
  ...
  result_ = baseline_randomforest_process(id_input_output_function_cli_params_)
...
workflow {
  def id = params.id
  def fname = "baseline_randomforest"
  ...
  def inputFiles = params.baseline_randomforest
    .arguments
  ...
  result = baseline_randomforest(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.baseline_randomforest.output = "baseline_randomforest.log"

  Channel.from(rootDir) \
    | filter { params.baseline_randomforest.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.baseline_randomforest.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | baseline_randomforest

  emit:
  baseline_randomforest.out
}