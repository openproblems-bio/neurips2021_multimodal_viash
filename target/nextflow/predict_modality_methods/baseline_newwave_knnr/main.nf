...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component baseline_newwave_knnr")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "baseline_newwave_knnr." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process baseline_newwave_knnr_process {
  label 'highmem'
  label 'midtime'
  label 'highcpu'
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.baseline_newwave_knnr.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow baseline_newwave_knnr {
  ...
  result_ = baseline_newwave_knnr_process(id_input_output_function_cli_params_)
...
workflow {
  def id = params.id
  def fname = "baseline_newwave_knnr"
  ...
  def inputFiles = params.baseline_newwave_knnr
    .arguments
  ...
  result = baseline_newwave_knnr(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.baseline_newwave_knnr.output = "baseline_newwave_knnr.log"

  Channel.from(rootDir) \
    | filter { params.baseline_newwave_knnr.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.baseline_newwave_knnr.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | baseline_newwave_knnr

  emit:
  baseline_newwave_knnr.out
}