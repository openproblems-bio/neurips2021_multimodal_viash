...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component python_starter_kit")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "python_starter_kit." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process python_starter_kit_process {
  label 'lowmem'
  label 'lowtime'
  label 'lowcpu'
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.python_starter_kit.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow python_starter_kit {
  ...
  result_ = python_starter_kit_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.python_starter_kit
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.python_starter_kit.output = "python_starter_kit.log"

  Channel.from(rootDir) \
    | filter { params.python_starter_kit.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.python_starter_kit.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | python_starter_kit

  emit:
  python_starter_kit.out
}