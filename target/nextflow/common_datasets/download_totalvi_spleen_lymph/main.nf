...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component download_totalvi_spleen_lymph")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "download_totalvi_spleen_lymph." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process download_totalvi_spleen_lymph_process {
  time '10 m'
  memory '10GB'
  tag "${id}"
...
workflow download_totalvi_spleen_lymph {
  ...
  def key = "download_totalvi_spleen_lymph"
  ...
  result_ = download_totalvi_spleen_lymph_process(id_input_output_function_cli_params_) \
...
workflow {
  def id = params.id
  def fname = "download_totalvi_spleen_lymph"
  ...
  def inputFiles = params.download_totalvi_spleen_lymph
    .arguments
  ...
  result = download_totalvi_spleen_lymph(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.download_totalvi_spleen_lymph.output = "download_totalvi_spleen_lymph.log"

  Channel.from(rootDir) \
    | filter { params.download_totalvi_spleen_lymph.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.download_totalvi_spleen_lymph.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | download_totalvi_spleen_lymph

  emit:
  download_totalvi_spleen_lymph.out
}