...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component download_azimuth_dataset")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "download_azimuth_dataset." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process download_azimuth_dataset_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.download_azimuth_dataset.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow download_azimuth_dataset {
  ...
  def key = "download_azimuth_dataset"
  ...
  result_ = download_azimuth_dataset_process(id_input_output_function_cli_params_)
...
workflow {
  def id = params.id
  def fname = "download_azimuth_dataset"
  ...
  def inputFiles = params.download_azimuth_dataset
    .arguments
  ...
  result = download_azimuth_dataset(ch_)
...
workflow test {
  ...
  main:
  params.test = true
  params.download_azimuth_dataset.output = "download_azimuth_dataset.log"

  Channel.from(rootDir) \
    | filter { params.download_azimuth_dataset.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.download_azimuth_dataset.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | download_azimuth_dataset

  emit:
  download_azimuth_dataset.out
}