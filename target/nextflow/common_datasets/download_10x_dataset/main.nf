...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component download_10x_dataset")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "download_10x_dataset." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process download_10x_dataset_process {
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.download_10x_dataset.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow {
  def id = params.id
  def fname = "download_10x_dataset"
  ...
  def inputFiles = params.download_10x_dataset
    .arguments
  ...
  result = download_10x_dataset(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.download_10x_dataset.output = "download_10x_dataset.log"

  Channel.from(rootDir) \
    | filter { params.download_10x_dataset.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.download_10x_dataset.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | download_10x_dataset

  emit:
  download_10x_dataset.out
}