...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component dummy_single_mod")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "dummy_single_mod." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process dummy_single_mod_process {
  time '10m'
  memory '20 GB'
  tag "${id}"
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.dummy_single_mod.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow dummy_single_mod {
  ...
  result_ = dummy_single_mod_process(id_input_output_function_cli_params_) \
...
workflow {
  ...
  def inputFiles = params.dummy_single_mod
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.dummy_single_mod.output = "dummy_single_mod.log"

  Channel.from(rootDir) \
    | filter { params.dummy_single_mod.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.dummy_single_mod.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | dummy_single_mod

  emit:
  dummy_single_mod.out
}