...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component dummy_random")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "dummy_random." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process dummy_random_process {
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.dummy_random.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow dummy_random {
  ...
  result_ = dummy_random_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.dummy_random
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.dummy_random.output = "dummy_random.log"

  Channel.from(rootDir) \
    | filter { params.dummy_random.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.dummy_random.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | dummy_random
