...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component baseline_pca")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "baseline_pca." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process baseline_pca_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.baseline_pca.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow baseline_pca {
  ...
  result_ = baseline_pca_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.baseline_pca
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.baseline_pca.output = "baseline_pca.log"

  Channel.from(rootDir) \
    | filter { params.baseline_pca.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.baseline_pca.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | baseline_pca
