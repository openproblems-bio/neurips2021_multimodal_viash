...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component dummy_zeros")
      println("exiting now...")
        exit 1
    }
  }
...
process dummy_zeros_process {
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.dummy_zeros.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow dummy_zeros {
  ...
  result_ = dummy_zeros_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.dummy_zeros
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.dummy_zeros.output = "dummy_zeros.log"

  Channel.from(rootDir) \
    | filter { params.dummy_zeros.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.dummy_zeros.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | dummy_zeros
