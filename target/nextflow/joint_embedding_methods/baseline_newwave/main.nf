...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component baseline_newwave")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "baseline_newwave." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process baseline_newwave_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.baseline_newwave.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow baseline_newwave {
  ...
  result_ = baseline_newwave_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.baseline_newwave
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.baseline_newwave.output = "baseline_newwave.log"

  Channel.from(rootDir) \
    | filter { params.baseline_newwave.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.baseline_newwave.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | baseline_newwave

  emit:
  baseline_newwave.out
}