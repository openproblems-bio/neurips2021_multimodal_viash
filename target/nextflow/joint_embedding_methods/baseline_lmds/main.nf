...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component baseline_lmds")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "baseline_lmds." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process baseline_lmds_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.baseline_lmds.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow {
  ...
  def inputFiles = params.baseline_lmds
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.baseline_lmds.output = "baseline_lmds.log"

  Channel.from(rootDir) \
    | filter { params.baseline_lmds.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.baseline_lmds.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | baseline_lmds
