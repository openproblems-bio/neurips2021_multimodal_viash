...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component baseline_umap")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "baseline_umap." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process baseline_umap_process {
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.baseline_umap.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow baseline_umap {
  ...
  result_ = baseline_umap_process(id_input_output_function_cli_params_) \
...
workflow {
  ...
  def inputFiles = params.baseline_umap
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.baseline_umap.output = "baseline_umap.log"

  Channel.from(rootDir) \
    | filter { params.baseline_umap.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.baseline_umap.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | baseline_umap
