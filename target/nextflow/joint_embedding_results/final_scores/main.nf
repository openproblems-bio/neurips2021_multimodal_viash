...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component final_scores")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "final_scores." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
// A process that filters out output_scores from the output Map
process filterOutput_scores {
  ...
  when:
    input.keySet().contains("output_scores")
  exec:
    output = input["output_scores"]

}

// A process that filters out output_summary from the output Map
process filterOutput_summary {
  ...
  output:
    tuple val(id), val(output), val(_params)
  when:
    input.keySet().contains("output_summary")
  exec:
    output = input["output_summary"]

}

// A process that filters out output_json from the output Map
process filterOutput_json {
  ...
  output:
    tuple val(id), val(output), val(_params)
  when:
    input.keySet().contains("output_json")
  exec:
    output = input["output_json"]
...
process final_scores_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.final_scores.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow final_scores {
  ...
  result_ = final_scores_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.final_scores
    .arguments
  ...
  result = final_scores(ch_)

  result \
    | filterOutput_scores \
    | view{ "Output for output_scores: " + it[1] }


  result \
    | filterOutput_summary \
    | view{ "Output for output_summary: " + it[1] }


  result \
    | filterOutput_json \
    | view{ "Output for output_json: " + it[1] }
...
workflow test {
  ...
  main:
  params.test = true
  params.final_scores.output = "final_scores.log"

  Channel.from(rootDir) \
    | filter { params.final_scores.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.final_scores.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | final_scores
