...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component normalize")
      println("exiting now...")
        exit 1
    }
  }
...
process normalize_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.normalize.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow normalize {
  ...
  result_ = normalize_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.normalize
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.normalize.output = "normalize.log"

  Channel.from(rootDir) \
    | filter { params.normalize.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.normalize.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | normalize
