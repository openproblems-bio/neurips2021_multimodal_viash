...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component cluster_celltype")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "cluster_celltype." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process cluster_celltype_process {
  label 'lowmem'
  label 'lowcpu'
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.cluster_celltype.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow cluster_celltype {
  ...
  result_ = cluster_celltype_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.cluster_celltype
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.cluster_celltype.output = "cluster_celltype.log"

  Channel.from(rootDir) \
    | filter { params.cluster_celltype.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.cluster_celltype.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | cluster_celltype

  emit:
  cluster_celltype.out
}