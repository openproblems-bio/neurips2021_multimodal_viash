...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component pseudotime_order")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "pseudotime_order." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process pseudotime_order_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.pseudotime_order.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow pseudotime_order {
  ...
  result_ = pseudotime_order_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.pseudotime_order
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.pseudotime_order.output = "pseudotime_order.log"

  Channel.from(rootDir) \
    | filter { params.pseudotime_order.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.pseudotime_order.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | pseudotime_order

  emit:
  pseudotime_order.out
}