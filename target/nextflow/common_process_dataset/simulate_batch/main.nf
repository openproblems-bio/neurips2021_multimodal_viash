...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component simulate_batch")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "simulate_batch." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process simulate_batch_process {
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.simulate_batch.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow simulate_batch {
  ...
  result_ = simulate_batch_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.simulate_batch
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.simulate_batch.output = "simulate_batch.log"

  Channel.from(rootDir) \
    | filter { params.simulate_batch.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.simulate_batch.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | simulate_batch
