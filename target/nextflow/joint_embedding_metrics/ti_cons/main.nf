...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component ti_cons")
      println("exiting now...")
        exit 1
    }
  }
...
process ti_cons_process {
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.ti_cons.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow ti_cons {
  ...
  result_ = ti_cons_process(id_input_output_function_cli_params_)
...
workflow test {
  ...
  main:
  params.test = true
  params.ti_cons.output = "ti_cons.log"

  Channel.from(rootDir) \
    | filter { params.ti_cons.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.ti_cons.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | ti_cons
