...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component check_format")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "check_format." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process check_format_process {
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.check_format.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow check_format {
  ...
  result_ = check_format_process(id_input_output_function_cli_params_) \
...
workflow {
  ...
  def inputFiles = params.check_format
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.check_format.output = "check_format.log"

  Channel.from(rootDir) \
    | filter { params.check_format.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.check_format.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | check_format
