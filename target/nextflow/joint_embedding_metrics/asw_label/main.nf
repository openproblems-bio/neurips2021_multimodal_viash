...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component asw_label")
      println("exiting now...")
        exit 1
    }
  }
...
process asw_label_process {
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.asw_label.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow asw_label {
  ...
  result_ = asw_label_process(id_input_output_function_cli_params_) \
...
workflow {
  ...
  def inputFiles = params.asw_label
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.asw_label.output = "asw_label.log"

  Channel.from(rootDir) \
    | filter { params.asw_label.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.asw_label.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | asw_label
