...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component rfoob")
      println("exiting now...")
        exit 1
    }
  }
...
process rfoob_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.rfoob.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow rfoob {
  ...
  result_ = rfoob_process(id_input_output_function_cli_params_)
...
workflow test {
  ...
  main:
  params.test = true
  params.rfoob.output = "rfoob.log"

  Channel.from(rootDir) \
    | filter { params.rfoob.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.rfoob.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | rfoob
