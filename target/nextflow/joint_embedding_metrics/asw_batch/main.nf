...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component asw_batch")
      println("exiting now...")
        exit 1
    }
  }
...
process asw_batch_process {
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.asw_batch.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow asw_batch {
  ...
  result_ = asw_batch_process(id_input_output_function_cli_params_) \
...
workflow {
  ...
  def inputFiles = params.asw_batch
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.asw_batch.output = "asw_batch.log"

  Channel.from(rootDir) \
    | filter { params.asw_batch.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.asw_batch.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | asw_batch
