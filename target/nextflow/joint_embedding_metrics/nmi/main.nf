...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component nmi")
      println("exiting now...")
        exit 1
    }
  }
...
process nmi_process {
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.nmi.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow nmi {
  ...
  result_ = nmi_process(id_input_output_function_cli_params_)
...
workflow test {
  ...
  main:
  params.test = true
  params.nmi.output = "nmi.log"

  Channel.from(rootDir) \
    | filter { params.nmi.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.nmi.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | nmi
