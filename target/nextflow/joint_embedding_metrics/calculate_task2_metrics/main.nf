...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component calculate_task2_metrics")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "calculate_task2_metrics." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process calculate_task2_metrics_process {
  tag "${id}"
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.calculate_task2_metrics.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow calculate_task2_metrics {
  ...
  def key = "calculate_task2_metrics"
  ...
  result_ = calculate_task2_metrics_process(id_input_output_function_cli_params_) \
...
workflow {
  def id = params.id
  def fname = "calculate_task2_metrics"
  ...
  def inputFiles = params.calculate_task2_metrics
    .arguments
  ...
  result = calculate_task2_metrics(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.calculate_task2_metrics.output = "calculate_task2_metrics.log"

  Channel.from(rootDir) \
    | filter { params.calculate_task2_metrics.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.calculate_task2_metrics.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | calculate_task2_metrics

  emit:
  calculate_task2_metrics.out
}