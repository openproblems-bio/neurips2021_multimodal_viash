...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component cc_cons")
      println("exiting now...")
        exit 1
    }
  }
...
workflow cc_cons {
  ...
  result_ = cc_cons_process(id_input_output_function_cli_params_)
...
workflow test {
  ...
  main:
  params.test = true
  params.cc_cons.output = "cc_cons.log"

  Channel.from(rootDir) \
    | filter { params.cc_cons.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.cc_cons.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | cc_cons
