...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component latent_mixing")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "latent_mixing." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process latent_mixing_process {
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.latent_mixing.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow latent_mixing {
  ...
  result_ = latent_mixing_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.latent_mixing
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.latent_mixing.output = "latent_mixing.log"

  Channel.from(rootDir) \
    | filter { params.latent_mixing.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.latent_mixing.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | latent_mixing
