...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component collect_method_metadata")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "collect_method_metadata." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process collect_method_metadata_process {
  tag "${id}"
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.collect_method_metadata.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow collect_method_metadata {
  ...
  def key = "collect_method_metadata"
  ...
  result_ = collect_method_metadata_process(id_input_output_function_cli_params_)
...
workflow {
  def id = params.id
  def fname = "collect_method_metadata"
  ...
  def inputFiles = params.collect_method_metadata
    .arguments
  ...
  result = collect_method_metadata(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.collect_method_metadata.output = "collect_method_metadata.log"

  Channel.from(rootDir) \
    | filter { params.collect_method_metadata.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.collect_method_metadata.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | collect_method_metadata

  emit:
  collect_method_metadata.out
}